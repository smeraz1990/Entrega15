...
if (args.iscluster == "cluster" && cluster.isPrimary) {
    ...
    cpus.map(() => {
     ...
     servidor = app.listen(args.puerto, () => { 
        console.log(`Servidor conectado puerto ${args.puerto} PID - ${process.pid}`) 
    })
 }
...
const registerStrategy = new LocalStrategy({passReqToCallback:true},
    ...
    app.get("/", routes.getRoot);
      ...
      app.get("/datos",(req,res)=>{
        console.log(`Estamos en el puerto ${args.puerto}`)
        res.send(`Aqui andamos parienton ${args.puerto}`)
    })
